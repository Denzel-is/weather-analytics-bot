...
# API = '8e9cf68cbd7790771fedf34c66c97c9f'
...
#     print(f"Запрос погоды для города: {city}")  # Для отладки
#     try:
...
#     city = message.text.strip().lower()
#     # Ваш код для получения текущей погоды

# def get_weather_forecast(message):
#     city = message.text.strip().lower()

#     # Предполагаем, что lat и lon уже определены. Нужно реализовать функцию получения этих данных.
#     lat, lon = get_lat_lon_from_city(city)

#     try:
#         res = requests.get(f'http://api.openweathermap.org/data/2.5/forecast/daily?lat={lat}&lon={lon}&cnt=16&appid={API}&units=metric')
#         if res.status_code == 200:
...
# # Остальные функции (get_weather_forecast, process_forecast_data, send_forecast_to_user...)
